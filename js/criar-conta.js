...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        facebookSignupBtn.addEventListener('click', () => handleSocialSignup('facebook'));
        appleSignupBtn.addEventListener('click', () => handleSocialSignup('apple'));
    ...
    function calculatePasswordStrength(password) {
        ...
        if (!hasLowercase) feedback.push('letras minúsculas');
        if (!hasUppercase) feedback.push('letras maiúsculas');
        if (!hasNumbers) feedback.push('números');
        if (!hasSpecialChars) feedback.push('símbolos especiais');
        if (!isLongEnough) feedback.push('pelo menos 8 caracteres');
    ...
    function handleSocialSignup(provider) {
        ...
        // Simular autenticação social (substituir por SDKs reais)
        setTimeout(() => {
            const socialUser = {
                name: `Usuário ${provider.charAt(0).toUpperCase() + provider.slice(1)}`,
                email: `usuario@${provider}.com`,
                avatar: `https://ui-avatars.com/api/?name=${provider}&background=09e331&color=000&size=200`
            };
            
            saveUserData(socialUser, `social_token_${provider}_${Date.now()}`);
            localStorage.setItem('loginMethod', provider);
            
            logSignupEvent(provider, true);
    ...
    function setSocialLoadingState(provider, loading) {
        ...
        if (loading) {
            ...
            btnText.textContent = `Cadastrar com ${provider.charAt(0).toUpperCase() + provider.slice(1)}`;
            btnIcon.className = `fab fa-${provider}`;
            btn.disabled = false;
        }
    }
